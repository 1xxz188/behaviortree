...
func TestEditorTypeBoundaries(t *testing.T) {
	...
	for _, target := range []string{"node", "field", "parameter", "definition"} {
		for _, value := range []any{"unknown", "", nil, float64(1), false, "missing"} {
			...
			p := map[string]any{"schemaVersion": 1, "generation": map[string]any{"package": "generated", "contextType": "any"}, "trees": []any{map[string]any{"id": "t", "root": "root", "nodes": []any{map[string]any{"id": "root", "type": "wait"}}}}}
			switch target {
			case "node":
