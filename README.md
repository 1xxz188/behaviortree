...
默认巡逻示例仅包含控制节点和等待节点，业务定义库为空。添加动作或条件后，在右侧点击“新建业务定义”，填写稳定定义 ID、显示名称、业务实现函数、参数及宿主事件，可保存并绑定当前节点。业务实现函数是同包中的手写 Go 函数，可被多个画布节点复用。左侧“管理定义”用于编辑已有定义。
...
业务节点区“更多”提供“复制全部定义 JSON”和“导出全部定义 JSON”，不受目录和筛选条件影响；单条定义右键也可复制或导出。单条与全部都输出可重新导入的 JSON 数组，保留完整 64 位整数，只包含业务声明。复制不可用时显示可手动复制的文本；下载默认命名为 `business-definition.json` 或 `business-definitions.json`。跨工程复用流程为：复制／导出 → 粘贴／读取文件 → 预览和处理冲突 → 应用 → 保存。
