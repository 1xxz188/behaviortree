...
单击节点库查看内置节点说明，单击 Go 业务节点打开定义窗口；将节点拖入画布即可添加实例。从节点端点拖拽连线，在右侧配置属性和调整子节点顺序。节点移动和自动布局只改变画布坐标。支持复制、删除、撤销重做、多树与子树引用、工程 JSON 导入导出、业务节点目录导入、保存和重开。
...
默认巡逻示例仅包含控制节点和等待节点，业务定义库为空。添加动作或条件后，在右侧点击“新建业务定义”，填写稳定定义 ID、显示名称、业务实现函数、参数及事件依赖，可保存并绑定当前节点。事件依赖只能选择工程“事件枚举”中已经声明的成员，定义和所有绑定节点共享同一份依赖。业务实现函数是同包中的手写 Go 函数，可被多个画布节点复用。左侧“管理”打开业务节点管理窗口，按“业务定义 / 目录 / 标签”分区维护。业务定义页支持名称、ID、Go 函数搜索以及种类和直属目录筛选，默认查看侧栏当前目录；新建与导入使用管理窗口选中的目录。分类修改即时应用且支持撤销，关闭窗口不会回滚，请保存工程以保留修改。

业务节点支持自定义嵌套目录及工程共享标签。每个定义归属一个目录，可关联多个标签；点击目录选中新增位置，节点和目录都可以拖动。拖到条目前后调整同级混合顺序，拖到目录中间移入并追加末尾；定义拖入画布仍创建节点实例。右击目录可新建子目录、重命名、移动及删除；只允许删除空目录。右击定义可移动、设置标签或删除。删除标签仅解除关联，不删除定义。分类操作支持撤销重做和保存，不改变代码生成结果。

“搜索节点”同时匹配业务定义名称、ID 和 Go 函数名。标签筛选要求定义包含全部所选标签，并与文字搜索组合；结果覆盖整个业务库，显示目录路径，点击“定位”回到目录树。筛选结果不支持拖放排序，可通过右键菜单移动。

“导入业务定义”读取自包含的 Schema 4 目录对象，包含业务定义、所引用的事件成员和来源工程的枚举功能注释；可选择 `examples/catalog.json` 或 `model.ExportCatalog(project)` 的输出。导入先解析预览，再处理定义、成员和整体注释的冲突，最后一次性应用并保存工程。不同事件发生同号 ID 冲突时会自动分配目标工程的新 ID，并同步业务定义引用；代码名冲突仍需映射或改名。旧 JSON 数组及定义中的字符串 `events` 字段会明确报错。导入定义不包含 Go 实现，也不会扫描 Go 源码；新定义进入打开弹窗时选中的目录，同 ID 替换保留原目录、标签和顺序。

管理窗口业务定义页的“导出”菜单提供复制和下载；单条定义右键或行内“更多”也可复制或导出。交换对象只携带实际导出定义引用的事件，每个事件保留稳定 ID 和成员注释。复制不可用时显示可手动复制的文本。跨工程复用流程为：复制／导出 → 粘贴／读取文件 → 预览和处理冲突 → 应用 → 保存。

当前工程格式为 `schemaVersion: 4`，旧 Schema 3 及更早工程不会自动转换。工程的 `events` 数组保存统一事件注册表，`eventEnumDescription` 保存整个事件枚举的功能注释，成员的 `description` 保存该项的字段注释；业务定义只保存 `eventIds`。事件 ID 使用规范的十进制字符串，从 `1` 递增；`nextEventId` 持久保存下一个可用编号，删除事件后不复用已分配编号。ID 创建后保持不变，改显示名或代码名不会换 ID。目录、标签和排序保存在工程的 `catalogOrganization` 中，仅完整工程导入导出携带分类信息。导入后业务声明已随工程保存，不依赖原始定义文件，也不会自动回写该文件或手写 Go 代码。

独立“事件枚举”入口可编辑整体注释与各成员注释；画布左上角常驻事件面板可折叠、搜索，点击事件行高亮当前树中引用它的节点。标题栏“枚举说明”和每行“注释”分别打开可关闭的完整多行说明，不改变高亮或节点选区。删除事件会一次清理整个工程的定义引用，可通过一次撤销恢复事件、注释和引用。说明查看与高亮只是视图状态；编辑注释会使工程待保存、生成源码过期。

参数声明支持填写可选的多行“注释”，以 `comment` 字段随工程和目录保存，并生成到 `glue.gen.go` 的 `<GoName>Params` 对应成员注释中；未填写时保留默认绑定说明。业务定义的“显示名称”会出现在参数结构体、生成的节点函数和业务函数骨架注释中。修改后需重新预览或生成代码；已复制到手写文件中的骨架不会自动改写。
...
节点 ID 可在属性面板中显式修改并提交，要求非空且在所属树内唯一，不同树可使用相同 ID；输入冲突时立即提示并禁用应用，按回车也不会生效。提交会同步根、连线和布局，并可整体撤销。新增和复制使用树内递增的数字字符串 ID（`1`、`2`、`3`…），已有 UUID 或自定义 ID 保持原样。打开、切换树或恢复历史时，根据该树现存数字 ID、根、连线和布局建立分配游标；后续分配不扫描节点集合，手动改为更大数字时同步推进游标。编号不保证连续，不是永久流水号，重载后已删除的最大编号可能复用。日常重命名修改“名称”即可；修改 ID 属于身份变更，不会自动延续外部旧 ID 关联。源节点由 TreeID + NodeID 标识，整数槽位仅属于本次编译结果，不会写回节点 ID。

行为树名称仅供展示，允许重复；树列表和子树选项同时展示 ID。行为树 ID 是工程内唯一、区分大小写的运行时入口主键，只能包含英文、数字、下划线，允许数字开头；不接受空值，也不会自动裁剪或转换。新建工程的首棵树和根节点 ID 均为 `1`。新增树按工程内最大数字树 ID 递增，子树悬挂引用也参与避让；节点编号在各树内独立递增。手动将 NodeID 或 TreeID 从 `2` 改成 `100` 后，下一个对应 ID 为 `101`。分配只访问缓存游标，不扫描整个工程；恢复工程后根据现存 ID 重建，可能复用已删除的最大编号。改名称、保存和重新生成均不会改变已有 ID。
...
左侧树列表中的 `ID:` 显示当前生效身份，右侧 ID 输入框允许暂存尚未应用的草稿；二者不同时会明确提示“ID 修改尚未应用”和当前生效值，应用后同步。列表最右侧数字是节点数量。右击树条目（或聚焦后按 Shift+F10）可重命名展示名称或删除目标树：重命名不改变 ID，删除需弹窗二次确认，且至少保留一棵树。两种操作均可撤销；操作未选中的树不会切换当前画布，删除后的子树悬挂引用由校验提示修复。

存在未应用的树 ID、节点 ID 或代码名草稿时，顶部显示“有未应用修改”，保存、导出、校验、预览和生成都会提示先应用或取消，不再静默使用旧身份并报告成功。“应用 ID”位于输入框紧下方；应用使工程变为未保存并使旧源码过期，保存后再生成才能更新磁盘中的常量和函数名。
...
更新页面时，Vite 构建会先清理 `web/dist` 中的旧资源；无需手动删除工程目录或浏览器存储。Go 服务内嵌构建时的页面，重新构建后须保存页面中的修改、重启服务，再用 `Ctrl+F5` 强制刷新。只清浏览器缓存无法更新仍在运行的旧程序，不提供旧前端资源兼容逻辑。
...
重复子树实例始终用入口树与引用点代码名消歧，例如 `btNodeCombatAttackViaPatrolEngage`，不会因增加引用而给旧实例重新编号。内部引用链仍保留分隔信息；超过 120 字符的调用链使用 12 位稳定摘要，碰撞明确报错。最终 MixedCaps 名称若碰撞，使用稳定身份摘要消歧；与业务声明冲突时追加 `Generated`。普通无冲突入口不附加哈希。树文件按定义归属：A 引用 B 时，B 的所有独立展开实例都位于 B 文件；A 增删对 B 的引用会改变 B 文件。公共 glue 保存全局槽位、参数类型、黑板访问器、程序元数据及分派；命名只在生成阶段处理，运行时仍为整数 `switch` 与直接函数调用。
...
字段绑定由参数的 `field` 指定稳定字段 ID，常量由 `value` 指定，二者不能同时出现。绑定字段变化会通知依赖该字段的节点；条件依赖外部业务状态时，应在工程注册事件、在业务定义中选择 `eventIds`，由宿主先更新状态再调用 `instance.Notify(behavior.EventHostStateChanged)`。`Notify` 不再接受字符串；未声明的数值事件会被拒绝，合法但未被当前树监听的事件是空操作。异步完成仍使用 `Complete(token, result)`。
