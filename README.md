...
默认巡逻示例仅包含控制节点和等待节点，业务定义库为空。添加动作或条件后，在右侧点击“新建业务定义”，填写稳定定义 ID、显示名称、业务实现函数、参数及宿主事件，可保存并绑定当前节点。业务实现函数是同包中的手写 Go 函数，可被多个画布节点复用。左侧“管理”打开业务节点管理窗口，按“业务定义 / 目录 / 标签”分区维护。业务定义页支持名称、ID、Go 函数搜索以及种类和直属目录筛选，默认查看侧栏当前目录；新建与导入使用管理窗口选中的目录。分类修改即时应用且支持撤销，关闭窗口不会回滚，请保存工程以保留修改。
...
“导入业务定义”默认粘贴 JSON 数组，提供“填入示例”；也可切换“从文件读取”，选择 `examples/catalog.json` 或 `model.ExportCatalog` 的输出。两种方式均先“解析预览”，查看新增、相同及冲突定义，处理冲突后“应用到工程”，最后保存工程。修改输入会清空旧预览和冲突选择。导入定义不包含 Go 实现，也不会扫描 Go 源码；新定义进入打开弹窗时选中的目录，同 ID 替换保留原目录、标签和顺序。

管理窗口业务定义页的“导出”菜单提供“复制全部定义 JSON”和“下载全部定义 JSON”，不受目录和筛选条件影响；单条定义右键或行内“更多”也可复制或导出。单条与全部都输出可重新导入的 JSON 数组，保留完整 64 位整数，只包含业务声明。复制不可用时显示可手动复制的文本；下载默认命名为 `business-definition.json` 或 `business-definitions.json`。跨工程复用流程为：复制／导出 → 粘贴／读取文件 → 预览和处理冲突 → 应用 → 保存。

当前工程格式为 `schemaVersion: 2`，不兼容版本 1，请创建新工程；独立业务定义 JSON 数组格式保持不变。目录、标签和排序保存在工程的 `catalogOrganization` 中，仅完整工程导入导出携带分类信息。导入后业务声明已随工程保存，不依赖原始定义文件，也不会自动回写该文件或手写 Go 代码。
