...
顶部常驻显示服务端工作目录绝对路径、当前文件和保存状态。路径旁的“打开目录”直接弹出 Windows 原生资源管理器风格的文件夹选择窗口，初始位置为当前工作目录；选择后切换实际工作目录并重新加载，等同于更换本次服务的 `--workspace`，重启服务时仍以启动参数为准。首次保存和“另存为”的“选择目录”也使用同一个系统窗口，文件名和覆盖确认在保存对话框中完成；跨目录保存成功后，后续保存和生成均使用新目录。取消、目录不可读或保存失败不会切换工作目录。有未保存修改时，切换提供“保存并切换 / 放弃修改 / 取消”；保存期间继续编辑会保留新修改并停止切换。原生目录窗口目前支持 Windows，需在运行 bttool 的本机桌面使用。

“新建”创建空白草稿。“导入”只读取 JSON 内容到未保存草稿，不直接修改源文件；无效 JSON 或工程格式错误会弹窗显示具体原因，并保留当前工程。之后若保存到源文件所在目录并确认覆盖同名文件，才会覆盖源文件。鼠标悬浮在“导入”按钮上可查看说明。

当前工程列表仅显示通过读取校验的行为树 JSON，过滤无关配置、损坏文件和超限文件；尚未连完的可编辑草稿仍可打开。保存时仍检查全部同名文件并要求确认覆盖。
...
源码面板通过文件选择器切换树文件和公共 glue，选中节点会切换到定义树文件并定位对应函数；复制和下载使用当前文件的实际名称。源码映射包含版本、文件清单、每文件 SHA-256 及节点位置，读取时校验整批产物；任一文件缺失、被修改或映射错误都会拒绝读取。生成产物只支持当前格式。保存按钮可独立保存草稿，“生成到目录”会先完成保存；后续校验或生成失败时，已保存的工程仍保留。生成源码不等于业务 Go 函数已实现或编译通过。
