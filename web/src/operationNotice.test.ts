...
function notice(t: TestContext) {
  ...
  const props = Vue.reactive({ message: "初始状态", failed: false, busy: false });
...
test("下载成功展示完整路径及可关闭提示", async t => {
  ...
  try {
    ...
    n.props.busy = true;
    await Vue.nextTick();
    n.props.message = "下载成功，业务骨架已保存到 E:/workspace/behavior/actions.go";
    await Vue.nextTick();
    assert.equal(n.state.visible.value, false);
    n.props.busy = false;
    await Vue.nextTick();
...
test("重复操作相同结果重新显示，失败使用错误提示", async t => {
  const n = notice(t);
  try {
    n.props.message = "校验通过，可以生成 Go 代码";
    await Vue.nextTick();
    n.state.dismiss();
    n.props.busy = true;
    await Vue.nextTick();
...
// 精确推进虚拟时钟，验证两秒消失和新消息重置计时，不进行真实等待。
test("提示两秒自动消失，新提示不会被旧计时器提前关闭", async t => {
  const n = notice(t);
  try {
    n.props.message = "已保存";
    await Vue.nextTick();
    ...
    n.props.message = "校验通过";
    await Vue.nextTick();
    t.mock.timers.tick(1);
    assert.equal(n.state.visible.value, true);
    t.mock.timers.tick(1998);
    assert.equal(n.state.visible.value, true);
    t.mock.timers.tick(1);
    assert.equal(n.state.visible.value, false);
    assert.doesNotMatch(await n.html(), /校验通过/);
  } finally { n.scope.stop(); }
});
