...
const dirty = computed(() => !!target.value && draft.value !== (target.value.comment ?? ""));
const activeNode = computed(() => visible.value ? target.value : undefined); // 任意浮层打开时隐藏顶部气泡。
const title = computed(() => target.value ? target.value.name || kinds[target.value.type]?.label || target.value.id : ""); // 与画布名称一致。
...
let drafts = new WeakMap<BTNode, string>(); // 收起后按对象身份保留草稿，不额外遍历节点。
...
async function open(node: BTNode, element: HTMLElement, pin: boolean) {
  if (target.value !== node) {
    hide();
    target.value = node;
    draft.value = drafts.get(node) ?? node.comment ?? "";
  }
  retain();
...
function show(node: BTNode, element: HTMLElement) {
  retain();
  ...
  if (!node.comment?.trim()) return hide();
  return open(node, element, false);
}

// 气泡和右键菜单共用显式编辑入口，不受自动悬浮开关影响。
function edit(node: BTNode, element: HTMLElement) {
...
function cancel() {
  ...
  draft.value = target.value.comment ?? "";
...
function apply() {
  ...
  if (!props.commit(props.tree, node, draft.value.trim())) return hide();
  drafts.delete(node);
...
watch(() => props.tree, reset, { flush: "sync" });
watch(() => props.disabled, disabled => { if (disabled) hide(); });
watch(() => props.autoOpen, enabled => { if (!enabled && !pinned.value) hide(); });
// 仅同步同一目标的外部提交，切换节点时不能用旧目标正文覆盖恢复的草稿。
watch(() => [target.value, target.value?.comment] as const, ([node, comment], [previousNode, previous]) => {
...
<template>
  <Teleport to="body">
    <section v-if="visible && target" id="node-comment-tooltip" ref="tooltip" class="node-comment-tooltip"
      :class="[`bridge-${bridgeSide}`, { 'is-maximized': maximized }]" role="dialog" aria-labelledby="node-comment-heading"
      ...
      <header><strong id="node-comment-heading" :title="title">{{ title }}</strong>
      ...
      <textarea ref="commentInput" v-model="draft" aria-label="节点注释内容" placeholder="填写节点注释…" rows="6" :disabled="disabled" />
