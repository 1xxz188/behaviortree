...
func TestStageDoesNotCopyGeneratedAndKeepsHelpers(t *testing.T) {
	// 最长 ID 的驼峰分词结果可能超过旧的 80 字节文件名限制。
	longTreeName := "tree_" + strings.Repeat("a_b", 40) + ".gen.go"
	src := t.TempDir()
	...
	for name, data := range map[string]string{
		"actions.go":  "package behavior\n",
		"glue.gen.go": "invalid old glue", "tree_gen.map.json": "old manifest",
		"helper/version.go": "package helper\nconst Marker=\"base\"\n", "helper/glue.gen.go": "package helper\n",
		"tree_handwritten.gen.go": "package behavior\n",
		"tree_main.gen.go":        "// Code generated by behaviortree. DO NOT EDIT.\ninvalid stale source",
		"tree_Shared.gen.go":      "// Code generated by behaviortree. DO NOT EDIT.\r\ninvalid stale source",
		longTreeName:              "// Code generated by behaviortree. DO NOT EDIT.\ninvalid stale source",
	} {
	...
	for _, name := range []string{"tree_gen.map.json", "glue.gen.go", "tree_main.gen.go", "tree_Shared.gen.go", longTreeName} {
		if _, err := os.Stat(filepath.Join(dst, name)); !os.IsNotExist(err) {
			t.Fatalf("copied stale generation: %s", name)
		}
	}
	// 命名不属于生成器的业务文件及子包文件必须完整保留。
	for _, name := range []string{"actions.go", "tree_handwritten.gen.go", "helper/glue.gen.go"} {
		if _, err := os.Stat(filepath.Join(dst, name)); err != nil {
			t.Fatalf("missing business file %s: %v", name, err)
		}
	}
