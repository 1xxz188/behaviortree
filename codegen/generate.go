...
func (g *generator) emitHeader(withTime bool) {
	g.line("// Code generated by behaviortree. DO NOT EDIT.")
	g.line("// 本文件使用原生 Go 分支与调用；业务函数请写在同包独立文件中。")
	g.line("package %s", g.packageName)
...
// TreeFileName 单遍将已校验的 ASCII 树 ID 转成小写下划线文件名，调用方须检查转换后的重名。
func TreeFileName(id string) string {
	var name strings.Builder
	name.Grow(len(id)*2 + len("tree_.gen.go"))
	name.WriteString("tree_")
	for i := 0; i < len(id); i++ {
		c := id[i]
		if c >= 'A' && c <= 'Z' {
			// 小写或数字后的大写开始新词；连续大写缩写仅在后接小写的新词前分隔。
			if i > 0 && id[i-1] != '_' && ((id[i-1] >= 'a' && id[i-1] <= 'z') ||
				(id[i-1] >= '0' && id[i-1] <= '9') ||
				(i+1 < len(id) && id[i+1] >= 'a' && id[i+1] <= 'z')) {
				name.WriteByte('_')
			}
			c += 'a' - 'A'
		}
